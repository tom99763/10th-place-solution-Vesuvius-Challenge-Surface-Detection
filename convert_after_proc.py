...
OUTPUT_DIR = './data/2st_stage_oof_repeat2_topo_proc'  # Output directory (None = same as input)
...
def convert_npz_to_npy_float16(input_dir, output_dir=None, remove_original=False):
    ...
    npz_files = list(input_dir.glob('*.npy'))
    if not npz_files:
    ...
    for npz_path in tqdm(npz_files, desc="Converting"):
        prob = np.load(str(npz_path))
        prob = topo_postprocess(prob)

        filename = str(npz_path).split('/')[-1]
