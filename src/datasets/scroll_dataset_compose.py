...
class ComposeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            skeleton_mask, edge_mask, cover_mask = data['Skeleton'], data['Edge'], data['Cover']
...
def collate_fn_val(batch):
    ...
    skeleton_masks = torch.stack([item[3] for item in batch], dim=0)
    edge_masks = torch.stack([item[4] for item in batch], dim=0)
    cover_masks = torch.stack([item[5] for item in batch], dim=0)
