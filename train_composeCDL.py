...
import random
import torch
from src.datasets.scroll_dataset_composeCDL import *
from tqdm import tqdm
import json
from src.models.composeCDL import *
from src.trainers.composeCDLTrainer import *
...
@hydra.main(config_path="./configs", config_name="config_composeCDL", version_base=None)
def run(cfg: DictConfig):
    ...
    for i in range(len(val_splits)):
        ...
        model = ComposeCDL(cfg)
        if cfg.petrained_ckpt_path != '':
            pl_model = ComposeCDLRefineModule.load_from_checkpoint(
            ...
            pl_model = ComposeCDLRefineModule(model, cfg)
